...
data[label_col] = data[label_col].astype(str).replace(" ", "_")
...
with torch.no_grad():
    ...
    for count, batch_df in enumerate(batched_data):
        ...
        batch = collate_tokens(
            [torch.cat((model.encode(tokens[from_col], tokens[to_col]), torch.ones(512, dtype=torch.long)))[:512]
             for tokens in batch_df.itertuples(index=False)], pad_idx=1)

        logprobs = model.predict(classification_head, batch, return_logits=True)
        batch_df[tuple_col] = logprobs.detach().cpu().tolist()
        batch_df[softmax_col] = softmax(logprobs.detach().cpu().numpy(), axis=1).tolist()
        batch_df[pred_col] = logprobs.detach().cpu().tolist()
...
preds_df = preds_df.rename(columns={from_col: "pep", to_col: "MHC", label_col: "Label",
